...
with open('nus_test.csv','rb') as f:
...
nus_data = train_data[indexes[0:30000],:]
nus_label = train_label[indexes[0:30000],:]
...
ind_nus_data = np.array(range(nsample))
num_train = int(nsample * 0.8)
num_test = nsample - num_train
np.random.shuffle(ind_nus_data)
train_fea = nus_data[ind_nus_data[0:num_train],:]
test_fea = nus_data[ind_nus_data[num_train:],:]
train_label = nus_label[ind_nus_data[0:num_train],:]
test_label = nus_label[ind_nus_data[num_train:],:]
...
import pickle
with open('nus_inductive_grid_search_08.pickle','wb') as f:
...
import pickle
with open('nus_inductive_grid_search_05.pickle','wb') as f:
       pickle.dump([parameters,auc_score_train_gridsearch,auc_score_test_gridsearch],f)
...
import pickle
with open('nus_inductive_grid_search_03.pickle','wb') as f:
       pickle.dump([parameters,auc_score_train_gridsearch,auc_score_test_gridsearch],f)
