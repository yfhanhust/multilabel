                ...
                for lambda3 in [0.001,0.01,0.1,1]:
                    for delta in [0.05,0.1,0.5,1]:
                        ...
                        for out_round in range(5):
                            ...
                            U_init_nl = np.random.random((train_data.shape[0],nrank))
                            V_init_nl = np.random.random((train_data.shape[1],nrank))
                            W_init_nl = np.random.random((train_label_masked.shape[0],nrank))
                            H_init_nl = np.random.random((train_label_masked.shape[1],nrank))
                            B_init_nl = np.random.random((2*no_of_freq,train_label_masked.shape[1]))
                            U,V,W,H,M = ComputePUV_Inductive_NonLinearTF(train_data,train_label_masked,U_init_nl,V_init_nl,W_init_nl,H_init_nl,B_init_nl,ffproj,fea_loc,label_loc,alpha,lambda0,lambda1,lambda2,lambda3,delta,nrank,5,3500)
                            ######## training error 
                            ...
                            single_train_auc.append(auc_score)
                            ##### test phase 
                            Y_test_reconstructed = test_func(test_data,ffproj,M)
                            ...
                            single_test_auc.append(auc_score_test)
                            
                            print 'train auc: ' + str(auc_score)
                            print 'test auc: ' + str(auc_score_test)
                        
                        train_auc_score.append(single_train_auc)
                        test_auc_score.append(single_test_auc)
