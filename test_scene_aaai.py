...
##### for debug use 
kx = 30
#beta * Ydiffference + alpha/2 * (W * W).mean() + gamma/2 * ((Z * Z).mean() + (B * B).mean())
beta = 100
alpha = 0.01
gamma = 100.
fea_fraction = 0.6
...
Z,B,W,b= CoEmbed(train_fea,train_label,fea_loc,label_loc,kx,alpha,beta,gamma)
### reconstruction error 
print acc_label(train_label,Z,B,label_loc)
'''
    ...
    for alpha in [0.01,0.1,1,10]:
        for gamma in [0.1,1,10,100]:
            ...
            for iround in range(10):
             ...
             auc_aaai_list.append(single_auc)

import pickle

result_file_name = 'scene_result_aaai.pickle'
