...
train_fea = scene_data
train_label = scene_label
...
import pickle 
result_file_name = 'scene_baseline_test.pickle'
...
train_label_masked = train_label.copy()
train_label_masked[label_loc] = 0. ### generate a positive-unlabelled matrix
for lambda0 in [0.01,0.1,0.5,1,5]:
    for lambda1 in [0.01,0.1,1]:
        for lambda2 in [0.01,0.1,1]:
            for delta in [0.1,1,5]:
                U,V,W,H = completionPUVReg(train_fea,train_label_masked,fea_loc,label_loc,alpha,lambda0,lambda1,lambda2,delta,kx)
...
'''
for lambda0 in [0.01,0.1,1,10]: #4 
    for lambda1 in [0.01,0.1,1,10]: #4
        for lambda2 in [0.01,0.1,1,10]: #4
            for lambda3 in [0.01,0.1,1,10]: #4 
                for delta in [0.1,1,5,10]: # 4
                    acc_one_round = [] 
                    for round in range(5):
                        ...
                        reconstruction = Y_reconstructed[label_loc].tolist()
                        auc_score = roc_auc_score(np.array(ground_truth),np.array(reconstruction))
                        acc_one_round.append(auc_score)
                        
                    para_list.append((lambda0,lambda1,lambda2,lambda3,delta,kx))
                    acc_list.append(np.mean(np.array(acc_one_round)))
                    print 'auc score: ' + str(np.mean(np.array(acc_one_round)))
'''